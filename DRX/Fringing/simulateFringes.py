...
def main(args):
	...
	# Read in the GSM model - special version for this with 32 sides (~1 
	# degree resolution).
	gsmDict = numpy.load('gsm_low.npz')
	i = 0
	rawSrcs = {}
	for (ra,dec), T, a in zip(gsmDict['eCoords'], gsmDict['T0'], gsmDict['alpha']):
		name = 'ptSource%i' % i
		rawSrcs[name] = aipy.amp.RadioFixedBody(ra, dec, name=name, jys=T, mfreg=gsmDict['f0']/1e3, index=a)
		i += 1
	print "Modeling sky using %i point sources" % i
	...
	simDict1 = simVis.buildSimData(aa1, rawSrcs, jd=jdList, pols=['xx',])
	simDict2 = simVis.buildSimData(aa2, rawSrcs, jd=jdList, pols=['xx',])
