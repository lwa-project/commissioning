    ...
    if (not os.path.exists("%s.npz" % base)) or args.force:
        ...
        for i in range(nChunks):
            ...
            for j in xrange(0, masterSpectra.shape[1], 4):
                ...
                freq, tempSpec = fxc.SpecMaster(tempData, LFFT=LFFT, window=window, verbose=args.verbose)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='given a TBW file, plot the time averaged spectra for each digitizer input and save the data for later review with smGUI as an NPZ file using less memory that stationMaster.py', 
