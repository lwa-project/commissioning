...
if __name__ == '__main__':
	...
	seed(1)
	data =  randn(nBL*nFq, nStand)            # the data array
	print data[:,0].shape, data[:,1:].shape, len(['tau%i' % (i+1) for i in xrange(data.shape[1]-1)])

	# intercept is added, by default
	m = ols(data[:,0],data[:,1:], y_varnm = 'deltaPhase',x_varnm = ['tau%i' % (i+1) for i in xrange(data.shape[1]-1)])
	m.summary()
